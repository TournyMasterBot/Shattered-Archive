...
/**
 * AI-ANNOTATION
 * @ai-summary Phase G: mud-builder's own delegated role store (Decision 4) —
 *   a JSON array of {accountId, tier} grants keyed by hub-introspected
 *   accountId, sibling to builder-auth.json under the same auth/ dir. No row
 *   for an account means tier 'user' (the ladder's floor) — this store is
 *   additive-only, never a gate on anything by its own absence.
 * @ai-public RoleStore, RoleGrant
 * @ai-notes Same corrupt-file-locks-rather-than-resets idiom as AuthStore, but
 *   an ABSENT file is just "no grants yet" (valid, common — not a first-run
 *   event needing anything generated), unlike the master key.
 */
...
export class RoleStore {
  ...
  /** Upserts a grant; the store itself does not enforce WHO may call this — routes/roles.ts does. */
