/**
 * Local Jest config for the simulacrum-server package.
 *
 ...
 * `-server` dir, so no co-located test below it can match. Like soulsteel-server, it
...
module.exports = {
  ...
  moduleNameMapper: {
    ...
    // ts-jest's CJS transform) — same reasoning as mud-builder-server's own mapping.
