...
import {
  ...
  getGlobalScriptsStorageKey,
  setGlobalScriptSource,
  type GlobalScriptLanguage,
} from '../features/userScripts/globalScriptsStore';
import { PLUGINS_STORAGE_KEY, readInstalledPlugins, writeInstalledPlugins } from '../hooks/usePlugins';
import {
  EXPORT_SCHEMA,
  extractGlobalsItem,
  extractPluginConfigsItem,
  isValidUserScript,
  tryParseExportFile,
  type ExportFileV1,
} from '../features/userScripts/exportFormat';
...
// ExportFileV1 and the whole parsing side now live in
// features/userScripts/exportFormat.ts — pure, and therefore unit-testable
// against real mobile-written files, which nothing in this .tsx can be (Jest
// cannot resolve its .scss import).

// ── AI-METHOD ── regenerated by annotate_methods — do not hand-edit
// @ai-method: nowIso — Returns current timestamp in ISO 8601 format as string.
// @ai-hash: bf1dc2b1
// ── END AI-METHOD ──
function nowIso() {
...
// ── AI-METHOD ── regenerated by annotate_methods — do not hand-edit
// @ai-method: safeFileStamp — Returns current date and time formatted as YYYYMMDD-HHMMSS string for filename use.
// @ai-hash: d24126d8
// ── END AI-METHOD ──
...
// ── AI-METHOD ── regenerated by annotate_methods — do not hand-edit
// @ai-method: downloadJson — Downloads JSON data as a file with the specified filename.
// @ai-hash: ccd9f156
// ── END AI-METHOD ──
...
// The pure format helpers (isObject, isValidUserScript, tryParseExportFile,
// extractGlobalsItem, extractPluginConfigsItem) moved to
// features/userScripts/exportFormat.ts so they can be unit-tested.
...
const ImportExportModal: React.FC<ImportExportModalProps> = ({
  ...
  // Held separately from importFile: tryParseExportFile intentionally returns
  // only the userScripts items, so the globals item is picked off the raw text.
  const [importFileGlobals, setImportFileGlobals] = useState<Partial<Record<GlobalScriptLanguage, string>> | null>(null);
  const [importFilePlugins, setImportFilePlugins] = useState<Record<string, unknown>[] | null>(null);
  ...
  useEffect(() => {
    ...
    setImportFileGlobals(extractGlobalsItem(importText));
    setImportFilePlugins(extractPluginConfigsItem(importText));
  ...
  // ── AI-METHOD ── regenerated by annotate_methods — do not hand-edit
  // @ai-method: toggleAllFiltered — Toggles the selection state of all filtered scripts to the specified boolean value.
  // @ai-hash: 417084e6
  // ── END AI-METHOD ──
  ...
  // ── AI-METHOD ── regenerated by annotate_methods — do not hand-edit
  // @ai-method: toggleOne — Updates the selection state for a specific ID with the given checked status.
  // @ai-hash: c5a3aed8
  // ── END AI-METHOD ──
  ...
  // ── AI-METHOD ── regenerated by annotate_methods — do not hand-edit
  // @ai-method: buildExport — Builds and returns a formatted JSON export string containing selected script data.
  // @ai-hash: 175ea6e3
  // ── END AI-METHOD ──
  const buildExport = () => {
    ...
    const file: ExportFileV1 = {
      ...
      items: [
        ...
        // Globals ride along: a trigger that calls runGlobal("global.lua.foo")
        // does nothing on the importing device without the file defining foo.
        {
          storage: 'localStorage',
          key: getGlobalScriptsStorageKey(connectionId),
          format: 'json',
          kind: 'globalScripts',
          selection: { ids: [] },
          strategyHint: 'replace',
          value: getGlobalScriptsSnapshot(connectionId),
        } as unknown as ExportFileV1['items'][number],
        // Plugin configuration rides along too. The plugin IDs are shared with
        // the mobile client (all 18 match), so a configured plugin carries over
        // between them; an ID the importing client does not ship is skipped
        // there rather than treated as an error.
        {
          storage: 'localStorage',
          key: PLUGINS_STORAGE_KEY,
          format: 'json',
          kind: 'pluginConfigs',
          selection: { ids: [] },
          strategyHint: 'replace',
          value: readInstalledPlugins(),
        } as unknown as ExportFileV1['items'][number],
      ],
    };
  ...
  // ── AI-METHOD ── regenerated by annotate_methods — do not hand-edit
  // @ai-method: handleExportDownload — Exports user scripts data to a JSON file with timestamped filename.
  // @ai-hash: e068e6eb
  // ── END AI-METHOD ──
  ...
  // ── AI-METHOD ── regenerated by annotate_methods — do not hand-edit
  // @ai-method: handleImportApply — Processes imported items, filters valid user scripts, and updates import results.
  // @ai-hash: f8fc23f1
  // ── END AI-METHOD ──
  const handleImportApply = () => {
    ...
    // Every selected item, regardless of its `key`. That key records the
    // connection the file was exported FROM; matching it against this
    // connection's storage key meant a file only ever imported back into the
    // exact connection it came from — which blocked mobile→web entirely and
    // web→web between two different connections.
    ...
    // Apply any globals carried in the file to THIS connection. Read off the
    // raw parsed file rather than importFile.items, which holds only the
    // userScripts items tryParseExportFile keeps.
    const rawGlobals = importFileGlobals;
    if (rawGlobals) {
      setGlobalScriptSource(connectionId, 'javascript', rawGlobals.javascript ?? '');
      setGlobalScriptSource(connectionId, 'typescript', rawGlobals.typescript ?? '');
      setGlobalScriptSource(connectionId, 'lua', rawGlobals.lua ?? '');
      setGlobalScriptSource(connectionId, 'python', rawGlobals.python ?? '');
    }

    // Apply any plugin configuration carried in the file, matched by plugin id
    // (which is what the two clients share). Records for plugins this client
    // does not have installed are skipped rather than added — an entry with no
    // backing module would show up as a phantom row on the Plugins page.
    if (importFilePlugins && importFilePlugins.length > 0) {
      try {
        const installed = readInstalledPlugins();
        const incomingById = new Map(importFilePlugins.map((r) => [String(r.id), r] as const));

        const merged = installed.map((local) => {
          const match = incomingById.get(String(local.id));
          if (!match) return local;
          return { ...local, enabled: !!match.enabled, userConfig: (match.userConfig ?? {}) as Record<string, unknown> };
        });

        // writeInstalledPlugins, not a raw localStorage write: it notifies every
        // mounted usePlugins() so the change lands without a page reload.
        writeInstalledPlugins(merged);
      } catch {
        // A failed plugin merge must not lose the script import that succeeded.
      }
    }
...
// ── AI-METHOD ── regenerated by annotate_methods — do not hand-edit
// @ai-method: UserScriptSandboxModal — Manages user script sandbox editing with triggers, aliases, timers, and global scripts in a modal interface.
// @ai-hash: dea36a10
// ── END AI-METHOD ──
