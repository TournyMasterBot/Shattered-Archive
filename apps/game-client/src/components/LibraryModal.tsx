...
type TabId = 'parchment' | 'notes' | 'books' | 'organizations' | 'colors' | 'scratch';
...
// Ported from dsl-client/components/book-editor/BookEditorScreen.tsx (tagKey/groupByTag) —
// no shared package exists across these repos, so this is a deliberate copy, not an import.
...
export const LibraryModal: React.FC<LibraryModalProps> = ({ isOpen, onClose, connectionId }) => {
  ...
  const buildExportBundle = (): LibraryExportBundle => {
    ...
    if (tab === 'parchment' && lib.notes.length > 0) {
      bundle.parchment = lib.notes.map(
        (n): ParchmentExport => ({ title: n.title, body: n.body, tag: n.tag, createdAt: n.createdAt, updatedAt: n.updatedAt }),
      );
  ...
  const handleImportJsonFile = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (tab === 'parchment' && bundle.parchment?.length) {
      for (const p of bundle.parchment) {
        ...
        await lib.saveNote({ ...created, title: p.title, body: p.body, tag: p.tag, updatedAt: p.updatedAt });
      }
  ...
  const parchmentByTag = React.useMemo(
    () => groupByTag(lib.notes, (n) => n.tag, (n) => n.title || 'Untitled'),
    [lib.notes],
  );

  const booksByTag = React.useMemo(
    () => groupByTag(lib.books, (b) => b.tag, (b) => b.title || 'Untitled'),
    [lib.books],
  );

  // Notes group by spool first (existing hierarchy), then by tag within each spool.
  const notesBySpool = React.useMemo(() => {
    ...
    const bySpool = Array.from(groups.entries()).sort((a, b) => a[0].localeCompare(b[0], undefined, { sensitivity: 'base' }));
    return bySpool.map(
      ([spool, items]): [NoteSpool, Array<[string, UserNote[]]>] => [
        spool,
        groupByTag(items, (n) => n.tag, (n) => n.subject || 'Untitled'),
      ],
    );
  }, [lib.userNotes]);
    ...
    <div className={styles.backdrop}>
      <div className={styles.modal}>
        ...
        <div className={styles.tabBar}>
          ...
          <button
            type="button"
            className={`${styles.tabButton} ${tab === 'organizations' ? styles.tabButtonActive : ''}`}
            onClick={() => handleSetTab('organizations')}
          >
            Organizations
          </button>
        ...
        <div className={styles.body}>
              ...
              <div className={styles.splitHeader}>
                ...
                <div className={styles.tagRow}>
                  <input
                    ...
                    placeholder="Tag (optional, for grouping)"
                    maxLength={60}
                  />
                </div>
              </div>
              ...
              <div className={styles.splitBody}>
                <div className={styles.listPane}>
                  <div className={styles.list}>
                    {tab === 'parchment' &&
                      parchmentByTag.map(([tagLabel, items]) => (
                        <div key={tagLabel} className={styles.treeGroup}>
                          <div className={styles.treeGroupHeader}>
                            {tagLabel} ({items.length})
                          </div>
                          {items.map((item) => (
                            <button
                              type="button"
                    ...
                    {tab === 'notes' &&
                      notesBySpool.map(([spool, tagGroups]) => (
                        <div key={spool} className={styles.treeGroup}>
                          <div className={styles.treeGroupHeader}>{spool}</div>

                          {tagGroups.map(([tagLabel, items]) => (
                            <div key={tagLabel}>
                              <div className={styles.treeSubGroupHeader}>
                                {tagLabel} ({items.length})
                              </div>
                              {items.map((n) => (
                                <button
                ...
                <div className={styles.editorPane}>
                  <div className={styles.editorMain}>
                    ...
                    >
                      {bodyStats.lineCount} line{bodyStats.lineCount === 1 ? '' : 's'}, line{' '}
                      {bodyStats.currentLineIndex + 1} is {bodyStats.currentLineLength} char
                      {bodyStats.currentLineLength === 1 ? '' : 's'} long
                    </div>
                  </div>
          ...
          {tab === 'organizations' && <OrganizationsPane />}
