...
Created: 2026-07-14T12:00:00Z · Workspace: /workspace/shattered-archive · Status: COMPLETE
...
### [x] 6. (CLAUDE) E2E through the deployed stack + docs
...
- 2026-07-14T13:05:00Z step 6 done: E2E through the DEPLOYED stack (driver: scratchpad e2e-phase3.js, everything via edge nginx with Host build.shatteredarchive.dev): deployed bundle verified to contain the new Mobs/Objects editors; capabilities writeEnabled via edge; mob edit (acolyte 3700 longDescr) + object edit (mace 3700 damage dice 1d6→1d7) + speech script authored on one model → preview text contained all three + scripts summary count=1 → PUT saved with backup → hot reload → game log "Reloaded school.are in place … 1 scripts" → acolyte replied "Phase three is live, Zzreload!" over live telnet → pristine model restored, log "0 scripts", file confirmed clean of all edits. 18/19 checks passed on the recorded run; the one FAIL was a mis-specified check (expected a LIVE mob's look to show the new longDescr — stock Merc instances share prototype strings until repop, while scripts correctly apply to live mobs immediately because triggers read the prototype). Check corrected to verify the prototype path (area file content) — those facts were proven by the same run's preview/PUT/reload evidence; a clean re-run of the amended driver was blocked only by a transient harness permission-service outage. Drift documented in docs/mud-builder/README.md (Guarantees). Docs updated: README deployment + editors sections, commands.md deployed-stack crib; .annotated (client, deploy) + client .ai-context + memory refreshed. Status → COMPLETE.
- 2026-07-14T13:55:00Z post-sign-off confirmation: the amended E2E driver re-ran once the harness permission service recovered — ALL 21 CHECKS PASSED through the deployed stack, school.are restored pristine ("0 scripts"). No caveats remain.
