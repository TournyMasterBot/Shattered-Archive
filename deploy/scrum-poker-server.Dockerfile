...
COPY apps/scrum-poker-server ./apps/scrum-poker-server
COPY types ./types
COPY utils ./utils
...
RUN pnpm --filter @shatteredarchive/scrum-poker-core build

RUN pnpm --filter @shatteredarchive/scrum-poker-server... build
...
# boot log clean and matches kingdom-tactics-server's runtime stage. Real per-service config
...
COPY --from=build /repo/apps/scrum-poker-server ./apps/scrum-poker-server
...
RUN pnpm install --frozen-lockfile --prod --filter @shatteredarchive/scrum-poker-server...

EXPOSE 63000
CMD ["node", "apps/scrum-poker-server/dist/index.js"]
