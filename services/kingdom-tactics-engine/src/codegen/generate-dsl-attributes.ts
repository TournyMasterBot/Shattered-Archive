...
// ── AI-METHOD ── regenerated by annotate_methods — do not hand-edit
// @ai-method: blockAfter — (no summary)
// @ai-hash: 2d6192d8
// ── END AI-METHOD ──
...
/**
 * Enum members of one named sub-array inside an `Affiliation { ... }` block, e.g.
 * `affiliationList(affBlock, 'Classes', 'MortalClass')` → the `MortalClass.<X>`
 * names listed in `Classes = new MortalClass[] { ... }`. Empty when the property
 * is absent. Scoped to the property's own `{ ... }` so sibling arrays don't bleed in.
 */
function affiliationList(affBlock: string, prop: string, enumName: string): string[] {
  const sub = blockAfter(affBlock, new RegExp(`\\b${prop}\\s*=\\s*new`));
  if (!sub) return [];
  return collect(sub, new RegExp(`${enumName}\\.(\\w+)`, 'g'));
}
...
// ── AI-METHOD ── regenerated by annotate_methods — do not hand-edit
// @ai-method: serialize — (no summary)
// @ai-hash: 39fa52ea
// ── END AI-METHOD ──
...
// ── AI-METHOD ── regenerated by annotate_methods — do not hand-edit
// @ai-method: emitTable — (no summary)
// @ai-hash: 940fb3bb
// ── END AI-METHOD ──
...
interface RaceAttributes {
  ...
  /** Per-class DAMAGE boost/gimp from the race's BoostedClasses (boost 10 / superboost 20 /
   * SUPERBOOST 30; gimps negative) — a damage modifier, NOT a casting affinity. */
  classBoosts: { classKey: string; boost: number }[];
  /** Named racial abilities/traits (e.g. Toughness, Berserk) — literal type names from RacialAbilities. */
  racialAbilities: string[];
}

// ── AI-METHOD ── regenerated by annotate_methods — do not hand-edit
// @ai-method: parseRace — (no summary)
// @ai-hash: c24d2ed6
// ── END AI-METHOD ──
function parseRace(src: string): RaceAttributes | null {
  ...
  // BoostedClasses = new IClassBoost[] { new IClassBoost(Warrior.Instance, 20, …), … } — the second
  // arg is a DAMAGE boost (10/20/30) or gimp (negative), per class. NOT a casting affinity.
  const classBoosts = [...src.matchAll(/new\s+IClassBoost\(\s*(\w+)\.Instance\s*,\s*(-?\d+)/g)].map((m) => ({
    classKey: m[1],
    boost: Number(m[2]),
  }));
  // RacialAbilities = new IAbility[] { new Toughness(), new Berserk(), Meditation.Instance, ... }
  // Collect the concrete type names (both `new X()` and `X.Instance` forms); drop the array-type token.
  const raBlock = blockAfter(src, /\bRacialAbilities\b/) ?? '';
  const racialAbilities = [...raBlock.matchAll(/new\s+(\w+)\s*\(|(\w+)\.Instance/g)]
    .map((m) => m[1] ?? m[2])
    .filter((n): n is string => Boolean(n) && n !== 'IAbility');
  return {
    key: keyM[1],
    baseStats: statsM
...
// ── AI-METHOD ── regenerated by annotate_methods — do not hand-edit
// @ai-method: parseAbilityDict — (no summary)
// @ai-hash: daf4df3b
// ── END AI-METHOD ──
...
// ── AI-METHOD ── regenerated by annotate_methods — do not hand-edit
// @ai-method: parseClass — (no summary)
// @ai-hash: 5db62fba
// ── END AI-METHOD ──
function parseClass(src: string, file: string): ClassAttributes | null {
  ...
  const key = keyM[1];
  // Enum refs appear bare (`StatAttributes.X`), `Constants.`-qualified, or `Core.Constants.`-qualified
  // across the source — accept any prefix or several classes lose their primary attr / armor / group.
  const primM = /PrimaryAttribute\s*=\s*(?:Core\.)?(?:Constants\.)?StatAttributes\.(\w+)/.exec(src);
  const secM = /SecondaryAttribute\s*=\s*(?:Core\.)?(?:Constants\.)?StatAttributes\.(\w+)/.exec(src);
  const armM = /ArmorType\s*=\s*(?:Core\.)?(?:Constants\.)?DslArmorType\.(\w+)/.exec(src);
  const cgStr = /ClassGroup\s*=\s*"([^"]+)"/.exec(src);
  const cgEnum = /ClassGroup\s*=\s*(?:Core\.)?(?:Constants\.)?MortalClass\.(\w+)\.ToString/.exec(src);
  const castMod = /CastingLevelModifier\s*=\s*([\d.]+)f?/.exec(src);
  // Restriction lists are written several ways across the source: `new Constants.MortalRaces[]`,
  // `new MortalRaces[]`, `new Constants.MortalRaces []` (space before brackets, e.g. Monk), and the
  // implicit `new[]`. Match `= new <anything up to the opening brace>` so none are silently dropped.
  const rrBlock = blockAfter(src, /RaceRestrictions\s*=\s*new\b[^;{]*/) ?? '';
  // CSR/reclass gating: the class's `Affiliation { Classes, Allegiances, Gods, Races }` block.
  // Classes = required base class(es); Allegiances = clan/kingdom/faction; Gods = religion;
  // Races = an ALLOW list (distinct from raceRestrictions, which forbids).
  const affBlock = blockAfter(src, /Affiliation\s*=\s*new\s+Affiliation\s*\(\s*\)/) ?? '';
  return {
    key,
    ...
    csr: {
      requiresClasses: affiliationList(affBlock, 'Classes', 'MortalClass'),
      requiresAllegiances: affiliationList(affBlock, 'Allegiances', 'AffilitionAllegiance'),
      requiresGods: affiliationList(affBlock, 'Gods', 'AffiliationGods'),
      requiresRaces: affiliationList(affBlock, 'Races', 'MortalRaces'),
    },
    abilities: [
...
function main(): void {
  ...
  const raceIface =
    ...
    `  readonly classBoosts: readonly { readonly classKey: string; readonly boost: number }[];\n` +
    `  readonly racialAbilities: readonly string[];\n` +
    `}\n\n`;
  write(
    'race-attributes.ts',
  ...
  const classIface =
    ...
    `  readonly csr: {\n` +
    `    readonly requiresClasses: readonly string[];\n` +
    `    readonly requiresAllegiances: readonly string[];\n` +
    `    readonly requiresGods: readonly string[];\n` +
    `    readonly requiresRaces: readonly string[];\n` +
    `  };\n` +
